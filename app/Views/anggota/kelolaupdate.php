    ...
    <div class="col-sm d-flex align-items-stretch">
        <div class="card w-100 border border-2">
            <div class="card-body p-4">
                ...
                <form action="/anggota/update/<?= $kelompok['id_kelompok'] ?>" method="post" id="formEdit" class="form-container">
                    <?= csrf_field() ?>
                    <div class="mb-3">
                        ...
                        <select class="form-select <?= isset(session()->get('errors')['id_siswa']) ? 'is-invalid' : '' ?>" id="dselect" name="id_siswa[]" aria-label="Floating label select example">
                            ...
                            <?php foreach ($ketua as $key => $value) : ?>
                                <option value="<?= $value['id_siswa'] ?>" <?= $value['id_siswa'] == $ketuaReal[0]['id_siswa'] ? 'selected' : '' ?>><?= $value['nama_siswa'] ?> - <?= $value['nis'] ?> - <?= $value['tahun_ajaran'] ?></option>
                            <?php endforeach; ?>
                        </select>
                        <input type="hidden" name="siswaReal[]" value="<?= $ketuaReal[0]['id_anggota'] ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= isset(session()->get('errors')['id_siswa']) ? 'is-invalid' : '' ?>" id="dselect<?= $i ?>" name="id_siswa[]" aria-label="Floating label select example">
                                ...
                                <?php foreach ($anggota as $key => $value) : ?>
                                    <option value="<?= $value['id_siswa'] ?>" <?php if (isset($anggotaReal[$i - 1]['id_siswa']) == true) : ?><?= $anggotaReal[$i - 1]['id_siswa'] == $value['id_siswa'] ? 'selected' : '' ?><?php endif; ?>><?= $value['nama_siswa'] ?> - <?= $value['nis'] ?> - <?= $value['tahun_ajaran'] ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?php if (isset($anggotaReal[$i - 1]['id_siswa']) == true) : ?>
                                <input type="hidden" name="siswaReal[]" value="<?= $anggotaReal[$i - 1]['id_anggota'] ?>">
                            <?php endif; ?>
