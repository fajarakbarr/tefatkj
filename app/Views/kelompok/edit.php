    ...
    <div class="col-sm d-flex align-items-stretch">
        <div class="card w-100 border border-2">
            <div class="card-body p-4">
                <h5 class="card-title fw-semibold">Form Edit Kelompok</h5>
                ...
                <form action="/kelompok/update/<?= $kelompok['id_kelompok'] ?>" method="post" id="formEdit">
                    <?= csrf_field() ?>
                    <div class="form-floating mb-3">
                        <input type="text" class="form-control <?= isset(session()->get('errors')['nama_kelompok']) ? 'is-invalid' : '' ?>" name="nama_kelompok" id="nama_kelompok" placeholder="Kelompok" value="<?= $kelompok['nama_kelompok'] ?>" autocomplete="off">
                    ...
                    <div class="mb-3">
                        <select class="form-select <?= isset(session()->get('errors')['id_tahun_ajaran']) ? 'is-invalid' : '' ?>" id="dselect" name="id_tahun_ajaran" aria-label="Floating label select example">
                            ...
                            <?php foreach ($tahun_ajaran as $key => $value) : ?>
                                <option value="<?= $value['id_tahun_ajaran'] ?>" <?= $value['id_tahun_ajaran'] == $kelompok['id_tahun_ajaran'] ? 'selected' : '' ?>><?= $value['tahun_ajaran'] ?></option>
                            <?php endforeach; ?>
                        </select>
